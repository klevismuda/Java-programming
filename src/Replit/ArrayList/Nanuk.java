...
import java.util.*;
/*
In the Eskimo language nanuk means polar bear. When the tribe men goes hunting they need to select a leader,
each men boasts on how many fish, elk or other animals he can hunt.
Then they vote who will lead them.
A successful hunt for the leader is when they managed to hunt as much as he boasted
and the hunting party managed to avoid a polar bear (nanuk) which can ruin a whole hunt.
Experienced hunters know of ways in the terrain to avoid polar bears, they use way stones to mark these areas.
The method nanuk has an ArrayList r which represents what the hunters found during the hunt.
Tt can be a number of successful catches or the word "nanuk".
...
return true if hunters had enough way_stones to avoid all nanuks.
And they returned with successful hunt catches which should be more or equal to how many leader boasted before the hunt.
    ...
    public static boolean nanuk(ArrayList<String> r,int way_stones,int boast) {
        ...
        for (String each : r){
            if (each.equals("nanuk")){
                ...
                sumOfCatches += Integer.parseInt(each);
            }
        }
        if (sumOfCatches >= boast && sumOfNanus <= way_stones){
            return true;
        }
        return false;
    }
