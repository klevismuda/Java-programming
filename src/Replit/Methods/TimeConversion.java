...
public class TimeConversion {
    ...
    public static void timeConversion(String s) {
        String newType = s.substring(0, s.length() - 2);
        String AMorPM = s.substring(s.length() - 2);
        if(AMorPM.equals("AM") && newType.startsWith("12")){
            System.out.println("00:00:00");
        } else if(AMorPM.equals("AM")){
            System.out.println(newType);
        } else {
            int hour = Integer.parseInt(newType.substring(0, 2));
            hour += 12;
            String newTime = "" + hour + newType.substring(2);
